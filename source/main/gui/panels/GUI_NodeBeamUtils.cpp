...
void NodeBeamUtils::DrawSpringDampTab(ActorPtr actor)
{
    ...
    if (actor->ar_nb_initialized)
    {
        ...
        ImGui::Text("%s:   %f (%f)", _LC("NodeBeamUtils", "Jitter"), actor->ar_nb_reference[3] / static_cast<int>(actor->ar_beams.size()), actor->ar_nb_reference[2]);
        ImGui::NextColumn();
        ...
        ImGui::Text("%s:   %f (%f)", _LC("NodeBeamUtils", "Jitter"), actor->ar_nb_optimum[3] / static_cast<int>(actor->ar_beams.size()), actor->ar_nb_optimum[2]);
        ImGui::Columns(1);
    }
}
