...
void Autopilot::setInertialReferences(NodeNum_t refl, NodeNum_t refr, NodeNum_t refb, NodeNum_t refc)
{
    ...
    ref_span = (m_actor->ar_nodes_RelPosition[refl] - m_actor->ar_nodes_RelPosition[ref_r]).length();
}
...
float Autopilot::getAilerons()
{
    ...
    if (ref_l && ref_r)
    {
        float rat = (m_actor->ar_nodes_RelPosition[ref_r].y - m_actor->ar_nodes_RelPosition[ref_l].y) / ref_span;
        float bank = 90.0;
        if (rat >= 1.0)
            bank = 90.0;
        else if (rat <= -1.0)
        ...
        {
            Vector3 vel = (m_actor->ar_nodes_Velocity[ref_l] + m_actor->ar_nodes_Velocity[ref_r]) / 2.0;
...
float Autopilot::getElevator()
{
    ...
    if (ref_l && ref_r && ref_b)
    {
        ...
        float current_vs = (m_actor->ar_nodes_Velocity[ref_l].y + m_actor->ar_nodes_Velocity[ref_r].y) / 2.0;
        float pitch_var = current_vs - m_actor->ar_nodes_Velocity[ref_b].y;
        if (mode_alt == ALT_VS)
        {
            if (mode_heading == HEADING_NAV)
            {
        ...
        if (mode_alt == ALT_FIXED)
        {
            ...
            float current_alt = (m_actor->ar_nodes_AbsPosition[ref_l].y + m_actor->ar_nodes_AbsPosition[ref_r].y) / 2.0;
            if (wanted_vs < 0)
...
float Autopilot::getThrottle(float thrtl, float dt)
{
    ...
    if (ref_l && ref_r)
    {
        ...
        float altitude = m_actor->ar_nodes_AbsPosition[ref_l].y;
        ...
        float gspd = 1.94384449 * ((m_actor->ar_nodes_Velocity[ref_l] + m_actor->ar_nodes_Velocity[ref_r]) / 2.0).length();
...
void Autopilot::gpws_update(float spawnheight)
{
    ...
    if (mode_gpws && ref_b)
    {
        float groundalt = App::GetGameContext()->GetTerrain()->getHeightAt(m_actor->ar_nodes_AbsPosition[ref_c].x, m_actor->ar_nodes_AbsPosition[ref_c].z);
        ...
        float height = (m_actor->ar_nodes_AbsPosition[ref_c].y - groundalt - spawnheight) * 3.28083f; //in feet!
        //skip height warning sounds when the plane is slower then ~10 knots
        if ((m_actor->ar_nodes_Velocity[ref_c].length() * 1.9685f) > 10.0f)
        {
        ...
        float yVel = m_actor->ar_nodes_Velocity[ref_c].y * 1.9685f;
...
void Autopilot::UpdateIls()
{
    if (!ref_l || !ref_r)
        return;
    Vector3 position = (m_actor->ar_nodes_AbsPosition[ref_l] + m_actor->ar_nodes_AbsPosition[ref_r]) / 2.0;
