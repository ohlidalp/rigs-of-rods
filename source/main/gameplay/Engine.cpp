...
Engine::Engine(float _min_rpm, float _max_rpm, float torque, float reverse_gear, float neutral_gear, std::vector<float> forward_gears, float diff_ratio, ActorPtr actor) :
    m_air_pressure(0.0f)
    , m_auto_cur_acc(0.0f)
...
void Engine::SetEngineOptions(float einertia, char etype, float eclutch, float ctime, float stime, float pstime, float irpm, float srpm, float maximix, float minimix, float ebraking)
{
...
void Engine::UpdateEngine(float dt, int doUpdate)
{
    ...
    {
        // gear hack
        float velocity = m_actor->ar_nodes_hot[0].Velocity.length();

        Vector3 hdir = m_actor->GetCameraDir();
        if (hdir != Vector3::ZERO)
        {
            velocity = hdir.dotProduct(m_actor->ar_nodes_hot[0].Velocity);
        }
...
void Engine::pushNetworkState(float rpm, float acc, float clutch, int gear, bool running, bool contact, char automode, char autoselect)
{
...
void Engine::shiftTo(int newGear)
{
...
void Engine::UpdateInputEvents(float dt)
{
    ...
    else
    {
        ...
        if (fabs(m_actor->ar_avg_wheel_speed) <= 1.0f)
        {
            ...
            float velocity = hdir.dotProduct(m_actor->ar_nodes_hot[0].Velocity);
            ...
            else if (velocity > -1.0f && brake < 0.5f && accl > 0.5f && this->getGear() < 0)
            {
                ...
                if (this->getAutoMode() == SimGearboxMode::AUTO)
                {
                    this->autoShiftSet(Engine::DRIVE);
                }
                else
                {
                    this->setGear(1);
