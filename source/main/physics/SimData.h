...
/// Node = a point mass in the Mass-Spring-Damper physics model;
...
struct node_aux_t
{
    int nda_source_id = -1;                  //!< Number in truck file, -1 for nodes generated by wheels/cinecam
    std::string nda_source_name;             //!< Name in truck file, only if defined with 'nodes2'
    RigDef::Keyword nda_source_keyword = RigDef::Keyword::INVALID; //!< Section which defined/generated this node.
    int nda_source_datapos = -1;             //!< Data index in `RigDef::Module::nodes`
    float nda_minimass = -1.f;               //!< minimum node mass in Kg
    float nda_initial_node_mass = -1.f;      //!< for scaling
    Ogre::Vector3 nda_initial_node_position; //!< for scaling - to keep resetting functional

    // Flags for mass recalculation
    bool            nda_loaded_mass = false;   //!< User defined attr; mass is calculated from 'globals/loaded-mass' rather than 'globals/dry-mass'
