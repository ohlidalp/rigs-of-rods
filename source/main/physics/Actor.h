...
public:
    ...
    Ogre::Vector3     GetCameraDir()                    { ROR_ASSERT(ar_state != ActorState::DISPOSED); return (ar_nodes_RelPosition[ar_cameras[0].camera_node_pos] - ar_nodes_RelPosition[ar_cameras[0].camera_node_dir]).normalisedCopy(); }
    Ogre::Vector3     GetCameraRoll()                   { ROR_ASSERT(ar_state != ActorState::DISPOSED); return (ar_nodes_RelPosition[ar_cameras[0].camera_node_pos] - ar_nodes_RelPosition[ar_cameras[0].camera_node_roll]).normalisedCopy(); }
    ...
    std::vector<Ogre::Vector3> ar_nodes_Velocity;                //!< Index = `NodeNum_t`; The live data, updated every physics tick
    std::vector<Ogre::Vector3> ar_nodes_Forces;                  //!< Index = `NodeNum_t`; The live data, updated every physics tick
