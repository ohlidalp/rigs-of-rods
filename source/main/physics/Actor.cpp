...
void Actor::recalculateNodeMasses()
{
    ...
    {
        ar_nodes[it->rp_beam->p2num].mass = 100.0f;
    }
            ...
            if (!it->ti_tied)
            {
                ...
                for (ActorPtr& actor : App::GetGameContext()->GetActorManager()->GetActors())
                {
                    ...
                    for (std::vector<ropable_t>::iterator itr = actor->ar_ropables.begin(); itr != actor->ar_ropables.end(); itr++)
                    {
                        ...
                        if (this == actor.GetRef() && itr->rb_nodenum == it->ti_beam->p1num)
                            continue;

                        // calculate the distance and record the nearest ropable
                        float dist = (actor->ar_nodes[it->ti_beam->p1num].AbsPosition - actor->ar_nodes[itr->rb_nodenum].AbsPosition).length();
                        if (dist < mindist)
                        {
                            mindist = dist;
        ...
        else if (mode == ActorLinkingRequestType::ROPE_TOGGLE) // Do this only for `ROPE_TOGGLE`
        {
            ...
            for (ActorPtr& actor : App::GetGameContext()->GetActorManager()->GetActors())
            {
                ...
                for (std::vector<ropable_t>::iterator itr = actor->ar_ropables.begin(); itr != actor->ar_ropables.end(); itr++)
                {
                    ...
                    // calculate the distance and record the nearest ropable
                    float dist = (ar_nodes[it->rp_beam->p1num].AbsPosition - actor->ar_nodes[itr->rb_nodenum].AbsPosition).length();
                    if (dist < mindist)
                    {
                        mindist = dist;
                        nearest_actor = actor;
        ...
        if ((mode != ActorLinkingRequestType::HOOK_UNLOCK && mode != ActorLinkingRequestType::HOOK_RESET) && it->hk_locked == UNLOCKED)
        {
            ...
            for (ActorPtr& actor : App::GetGameContext()->GetActorManager()->GetActors())
            {
                ...
                if (nearest_node != NODENUM_INVALID)
                {
                    ...
                    {
                        it->hk_beam->p2num = it->hk_locked_node;
