...
Turbojet::Turbojet(ActorPtr actor, NodeNum_t tnodefront, NodeNum_t tnodeback, NodeNum_t tnoderef, RigDef::Turbojet & def)
{
    ...
    m_axis = m_actor->ar_nodes_RelPosition[m_node_front] - m_actor->ar_nodes_RelPosition[m_node_back];
...
void Turbojet::updateForces(float dt, int doUpdate)
{
    ...
    if (m_reverse)
        m_actor->ar_nodes_Forces[m_node_back] -= (enginethrust * 1000.0) * m_axis;
    else
        m_actor->ar_nodes_Forces[m_node_back] += (enginethrust * 1000.0) * m_axis;
