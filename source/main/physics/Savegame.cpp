...
bool ActorManager::SaveScene(Ogre::String filename)
{
    ...
    for (ActorPtr actor : x_actors)
    {
        ...
        for (int i = 0; i < actor->ar_num_nodes; i++)
        {
            ...
            // Velocity
            j_node.PushBack(actor->ar_nodes_hot[i].Velocity.x, j_doc.GetAllocator());
            j_node.PushBack(actor->ar_nodes_hot[i].Velocity.y, j_doc.GetAllocator());
            j_node.PushBack(actor->ar_nodes_hot[i].Velocity.z, j_doc.GetAllocator());
...
void ActorManager::RestoreSavedState(ActorPtr actor, rapidjson::Value const& j_entry)
{
    ...
    for (rapidjson::SizeType i = 0; i < nodes.Size(); i++)
    {
        ...
        actor->ar_nodes_hot[i].Velocity         = Vector3(data[3].GetFloat(), data[4].GetFloat(), data[5].GetFloat());
