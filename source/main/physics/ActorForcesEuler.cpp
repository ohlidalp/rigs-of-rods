...
void Actor::CalcForceFeedback(bool doUpdate)
{
    if (this == App::GetGameContext()->GetPlayerActor().GetRef())
    {
        ...
        {
            m_force_sensors.accu_body_forces += ar_nodes_hot[ar_camera_node_pos[ar_current_cinecam]].Forces;
        }
...
void Actor::CalcMouse()
{
    if (m_mouse_grab_node != NODENUM_INVALID)
    {
        ...
        ar_nodes_hot[m_mouse_grab_node].Forces += m_mouse_grab_move_force * dir;
    }
}
...
void Actor::CalcFuseDrag()
{
    ...
    {
        node_hot_t& nodefront = ar_nodes_hot[m_fusealge_front];
        node_hot_t& nodeback = ar_nodes_hot[m_fusealge_back];
        ...
        float altitude = nodefront.RelPosition.y + ar_origin.y;
...
void Actor::CalcBuoyance(bool doUpdate)
{
    ...
    if (m_buoyance->buoy_total_steps % BUOYANCY_SAMPLING_INTERVAL == 0)
    {
        ...
        for (size_t i = 0; i < m_buoyance->buoy_cached_nodes.size(); ++i)
        {
            ...
            m_buoyance->buoy_cached_nodes[i].Velocity    = ar_nodes_hot[nodenum].Velocity;
            m_buoyance->buoy_cached_nodes[i].Forces      = Ogre::Vector3::ZERO;

            // Projected node: advance position by N steps
            m_buoyance->buoy_projected_nodes[i].AbsPosition = ar_nodes[nodenum].AbsPosition + ar_nodes_hot[nodenum].Velocity * timeshift_sec;
            m_buoyance->buoy_projected_nodes[i].Velocity = ar_nodes_hot[nodenum].Velocity;
        ...
        {
            ar_nodes_hot[bcn.nodenum].Forces += bcn.Forces;
        }
    ...
    else
    {
        ...
        for (size_t i = 0; i < m_buoyance->buoy_cached_nodes.size(); ++i)
        {
            ...
            ar_nodes_hot[nodenum].Forces += interp_force;
        }
    }
...
void Actor::CalcWheels(bool doUpdate, int num_steps)
{
    ...
    for (int i = 0; i < ar_num_wheels; i++)
    {
        ...
        float relspeed = ar_nodes_hot[0].Velocity.dotProduct(getDirection());
        ...
        node_hot_t& armnode = ar_nodes_hot[ar_wheels[i].wh_arm_nodenum];
        node_hot_t& nearnode = ar_nodes_hot[ar_wheels[i].wh_near_attach_nodenum];
        ...
        {
            node_hot_t* outer_node = &ar_nodes_hot[ar_wheels[i].wh_tire_nodes[j]];
            node_hot_t* inner_node = (j % 2) ? (&ar_nodes_hot[ar_wheels[i].wh_axis_node1num]) : (&ar_nodes_hot[ar_wheels[i].wh_axis_node0num]);
            ...
            (&ar_nodes_hot[ar_wheels[i].wh_tire_nodes[j]])->Forces += dir * axis_precalc * inverted_rlen;
...
void Actor::CalcCommands(bool doUpdate)
{
    if (m_has_command_beams)
    {
        ...
        for (int i = 0; i < ar_num_rotators; i++)
        {
            ...
            Vector3 ax1 = ar_nodes_hot[ar_rotators[i].axis1].RelPosition;
            Vector3 ax2 = ar_nodes_hot[ar_rotators[i].axis2].RelPosition;
            ...
            for (int k = 0; k < 2; k++)
            {
                ...
                Vector3 ref1 = pl.projectVector(ax1 - ar_nodes_hot[ar_rotators[i].nodes1[k]].RelPosition);
                Vector3 ref2 = pl.projectVector(ax2 - ar_nodes_hot[ar_rotators[i].nodes2[k]].RelPosition);
                ...
                ar_nodes_hot[ar_rotators[i].nodes1[k    ]].Forces += (aerror * ref1len * rigidity) * dir1;
                ar_nodes_hot[ar_rotators[i].nodes2[k    ]].Forces -= (aerror * ref2len * rigidity) * dir2;
                // symmetric
                ar_nodes_hot[ar_rotators[i].nodes1[k + 2]].Forces -= (aerror * ref1len * rigidity) * dir1;
                ar_nodes_hot[ar_rotators[i].nodes2[k + 2]].Forces += (aerror * ref2len * rigidity) * dir2;
            }
        }
    }
}
...
void CheckBeamDeformation(Actor* actor, const int i, const float k, float& slen, float& len, const float difftoBeamL)
{
    if (actor->ar_beams[i].bm_type == BEAM_NORMAL && actor->ar_beams[i].bounded != SHOCK1 && k != 0.0f)
    {
        ...
        if (slen > actor->ar_beams[i].maxposstress && difftoBeamL < 0.0f) // compression
        {
            Real yield_length = actor->ar_beams[i].maxposstress / k;
            Real deform = difftoBeamL + yield_length * (1.0f - actor->ar_beams[i].plastic_coef);
            Real Lold = actor->ar_beams[i].L;
            actor->ar_beams[i].L += deform;
            actor->ar_beams[i].L = std::max(MIN_BEAM_LENGTH, actor->ar_beams[i].L);
            slen = slen - (slen - actor->ar_beams[i].maxposstress) * 0.5f;
            len = slen;
            if (actor->ar_beams[i].L > 0.0f && Lold > actor->ar_beams[i].L)
            {
                actor->ar_beams[i].maxposstress *= Lold / actor->ar_beams[i].L;
                actor->ar_beams[i].minmaxposnegstress = std::min(actor->ar_beams[i].maxposstress, -actor->ar_beams[i].maxnegstress);
                actor->ar_beams[i].minmaxposnegstress = std::min(actor->ar_beams[i].minmaxposnegstress, actor->ar_beams[i].strength);
            }
            // For the compression case we do not remove any of the beam's
            // strength for structure stability reasons
            //actor->ar_beams[i].strength += deform * k * 0.5f;
            if (actor->ar_beam_deform_debug_enabled)
            {
                RoR::Str<300> msg;
                msg << "[RoR|Diag] YYY Beam " << i << " just deformed with extension force "
                    << len << " / " << actor->ar_beams[i].strength << ". ";
                LogBeamNodes(msg, actor->ar_beams[i]);
                RoR::Log(msg.ToCStr());
            }
        }
        else if (slen < actor->ar_beams[i].maxnegstress && difftoBeamL > 0.0f) // expansion
        {
            Real yield_length = actor->ar_beams[i].maxnegstress / k;
            Real deform = difftoBeamL + yield_length * (1.0f - actor->ar_beams[i].plastic_coef);
            Real Lold = actor->ar_beams[i].L;
            actor->ar_beams[i].L += deform;
            slen = slen - (slen - actor->ar_beams[i].maxnegstress) * 0.5f;
            len = -slen;
            if (Lold > 0.0f && actor->ar_beams[i].L > Lold)
            {
                actor->ar_beams[i].maxnegstress *= actor->ar_beams[i].L / Lold;
                actor->ar_beams[i].minmaxposnegstress = std::min(actor->ar_beams[i].maxposstress, -actor->ar_beams[i].maxnegstress);
                actor->ar_beams[i].minmaxposnegstress = std::min(actor->ar_beams[i].minmaxposnegstress, actor->ar_beams[i].strength);
            }
            actor->ar_beams[i].strength -= deform * k;
            if (actor->ar_beam_deform_debug_enabled)
            {
                RoR::Str<300> msg;
                msg << "[RoR|Diag] YYY Beam " << i << " just deformed with extension force "
...
void CheckBeamBreaking(Actor* actor, const int i, const float k, float& slen, const float len, const float difftoBeamL)
{
    ...
    if (len > actor->ar_beams[i].strength)
    {
        ...
        SOUND_MODULATE(actor->ar_instance_id, SS_MOD_BREAK, 0.5 * k * difftoBeamL * difftoBeamL);
        SOUND_PLAY_ONCE(actor->ar_instance_id, SS_TRIG_BREAK);
        ...
        //connected to it.
        if (!((actor->ar_nodes[actor->ar_beams[i].p1num].nd_cab_node && actor->GetNumActiveConnectedBeams(actor->ar_beams[i].p1num) < 3) || (actor->ar_nodes[actor->ar_beams[i].p2num].nd_cab_node && actor->GetNumActiveConnectedBeams(actor->ar_beams[i].p2num) < 3)))
        {
            slen = 0.0f;
            ...
            {
                RoR::Str<200> msg;
                msg << "[RoR|Diag] XXX Beam " << i << " just broke with force " << len << " / " << actor->ar_beams[i].strength << ". ";
                LogBeamNodes(msg, actor->ar_beams[i]);
            ...
            if (actor->ar_beams[i].detacher_group > 0)
            {
                ...
                for (int j = 0; j < actor->ar_num_beams; j++)
                {
                    ...
                    if (abs(actor->ar_beams[j].detacher_group) == actor->ar_beams[i].detacher_group)
                    {
                        ...
                        if (actor->ar_beam_break_debug_enabled)
                        {
                            App::GetConsole()->putMessage(Console::CONSOLE_MSGTYPE_ACTOR, Console::CONSOLE_SYSTEM_NOTICE,
                                "Deleting Detacher BeamID: " + TOSTRING(j) + ", Detacher Group: " + TOSTRING(actor->ar_beams[i].detacher_group)+ ", actor ID: " + TOSTRING(actor->ar_instance_id));
                        }
                    }
                }
                ...
                {
                    if (wheeldetacher.wd_detacher_group == actor->ar_beams[i].detacher_group)
                    {
                        actor->ar_wheels[wheeldetacher.wd_wheel_id].wh_is_detached = true;
                        if (actor->ar_beam_break_debug_enabled)
                        {
                            App::GetConsole()->putMessage(Console::CONSOLE_MSGTYPE_ACTOR, Console::CONSOLE_SYSTEM_NOTICE,
                                "Detaching wheel ID: " + TOSTRING(wheeldetacher.wd_wheel_id) + ", Detacher Group: " + TOSTRING(actor->ar_beams[i].detacher_group)+ ", actor ID: " + TOSTRING(actor->ar_instance_id));
                        }
                    }
                }
            }
        ...
        {
            actor->ar_beams[i].strength = 2.0f * actor->ar_beams[i].minmaxposnegstress;
        }

        // something broke, check buoyant hull
        for (int mk = 0; mk < actor->ar_num_buoycabs; mk++)
        {
            int tmpv = actor->ar_buoycabs[mk] * 3;
            if (actor->ar_buoycab_types[mk] == Buoyance::BUOY_DRAGONLY)
                continue;
            if ((actor->ar_beams[i].p1num == actor->ar_cabs[tmpv] || actor->ar_beams[i].p1num == actor->ar_cabs[tmpv + 1] || actor->ar_beams[i].p1num == actor->ar_cabs[tmpv + 2]) &&
                (actor->ar_beams[i].p2num == actor->ar_cabs[tmpv] || actor->ar_beams[i].p2num == actor->ar_cabs[tmpv + 1] || actor->ar_beams[i].p2num == actor->ar_cabs[tmpv + 2]))
            {
                actor->m_buoyance->sink = true;
            }
        }
    }
}
...
void Actor::CalcBeamsInterActor()
{
    for (int i = 0; i < static_cast<int>(ar_inter_beams.size()); i++)
    {
        ...
        {
            const NodeNum_t p1num = ar_inter_beams[i]->p1num;
            node_hot_t& node_p1 = ar_nodes_hot[p1num];
            const Ogre::Vector3& pos_p1 = node_p1.RelPosition + ar_origin;
            const NodeNum_t p2num = ar_inter_beams[i]->p2num;
            node_hot_t& node_p2 = ar_inter_beams[i]->bm_locked_actor->ar_nodes_hot[p2num];
            const Ogre::Vector3& pos_p2 = node_p2.RelPosition + ar_inter_beams[i]->bm_locked_actor->ar_origin;

            // Calculate beam length
            Vector3 dis = pos_p1 - pos_p2;
            ...
            if (len > ar_inter_beams[i]->minmaxposnegstress)
            {
                if (ar_inter_beams[i]->bm_type == BEAM_NORMAL && ar_inter_beams[i]->bounded != SHOCK1 && k != 0.0f)
                {
                    ...
                    if (slen > ar_inter_beams[i]->maxposstress && difftoBeamL < 0.0f) // compression
                    {
                        ...
                        if (ar_beam_deform_debug_enabled)
                        {
                            RoR::Str<300> msg;
                            msg << "[RoR|Diag] YYY Beam " << i << " just deformed with extension force "
                ...
                if (len > ar_inter_beams[i]->strength)
                {
                    ...
                    // NOTE: cabnode check omitted, as inter-actor beams can't be.
                    slen = 0.0f;
    ...
    for (NodeNum_t i = 0; i < ar_num_nodes; i++)
    {
        ...
        if (!ar_nodes[i].nd_no_ground_contact)
        {
            ...
            if (ar_nodes[i].nd_has_ground_contact || ar_nodes[i].nd_has_mesh_contact)
            {
                ...
                ar_nodes_hot[i].RelPosition += ar_nodes[i].AbsPosition - oripos;
            }
        }
        ...
        if (i == ar_main_camera_node_pos)
        {
            ...
            m_camera_gforces_accu += ar_nodes_hot[i].Forces / ar_nodes[i].mass;
        }

        // integration
        if (!ar_nodes[i].nd_immovable)
        {
            ar_nodes_hot[i].Velocity += ar_nodes_hot[i].Forces / ar_nodes[i].mass * PHYSICS_DT;
            ar_nodes_hot[i].RelPosition += ar_nodes_hot[i].Velocity * PHYSICS_DT;
            ar_nodes[i].AbsPosition = ar_origin;
            ar_nodes[i].AbsPosition += ar_nodes_hot[i].RelPosition;
        }

        // prepare next loop (optimisation)
        // we start forces from zero
        // start with gravity
        ar_nodes_hot[i].Forces = Vector3(0, ar_nodes[i].mass * gravity, 0);

        Real approx_speed = approx_sqrt(ar_nodes_hot[i].Velocity.squaredLength());
        ...
        if (water)
        {
            ...
            if (is_under_water)
            {
                ...
                if (ar_num_buoycabs == 0)
                {
                    ...
                    ar_nodes_hot[i].Forces -= (DEFAULT_WATERDRAG * approx_speed) * ar_nodes_hot[i].Velocity;
                    // basic buoyance
                    ar_nodes_hot[i].Forces += ar_nodes[i].buoyancy * Vector3::UNIT_Y;
                }
                // engine stall
...
void Actor::CalcRopes()
{
    for (auto r : ar_ropes)
    {
        ...
        {
            node_hot_t& locked_node = r.rp_locked_actor->ar_nodes_hot[r.rp_locked_ropable->rb_nodenum];
            const Ogre::Vector3 locked_node_abs_pos = locked_node.RelPosition + r.rp_locked_actor->ar_origin;
            node_hot_t& rope_node = ar_nodes_hot[r.rp_beam->p2num];

            ar_nodes[r.rp_beam->p2num].AbsPosition = locked_node_abs_pos;
            rope_node.RelPosition = locked_node_abs_pos - ar_origin;
