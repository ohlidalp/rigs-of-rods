...
void SlideNode::UpdateForces(float dt)
{
    ...
    if (m_sliding_beam != BEAMID_INVALID || m_actor->ar_beams[m_sliding_beam].bm_broken || sn_slide_broken)
    {
        return;
    }
    ...
    m_actor->ar_nodes[m_actor->ar_beams[m_sliding_beam].p1num].Forces += perpForces * (1 - m_node_forces_ratio);
    m_actor->ar_nodes[m_actor->ar_beams[m_sliding_beam].p2num].Forces += perpForces * m_node_forces_ratio;
}

RailSegment* RailGroup::FindClosestSegment(ActorPtr& actor, const Ogre::Vector3& point)
{
    float closest_dist_sq = SlideNode::getLenTo(actor, &this->rg_segments[0], point);
    size_t closest_seg = 0;

    for (size_t i = 1; i < this->rg_segments.size(); ++i)
    {
        const float dist_sq = SlideNode::getLenTo(actor, &this->rg_segments[i], point);
...
RailSegment* RailSegment::CheckCurSlideSegment(ActorPtr& actor, const Ogre::Vector3& point)
{
    float closest_dist_sq = SlideNode::getLenTo(actor, this, point);
    RailSegment* closest_seg = this;

    if (this->rs_prev != nullptr)
    {
        const float dist_sq = SlideNode::getLenTo(actor, this->rs_prev, point);
        if (dist_sq < closest_dist_sq)
        {
    ...
    {
        const float dist_sq = SlideNode::getLenTo(actor, this->rs_next, point);
        if (dist_sq < closest_dist_sq)
        {
...
{
    // only do calcs if we have a beam to slide on
    if (m_sliding_beam != BEAMID_INVALID || m_actor->ar_beams[m_sliding_beam].bm_broken)
    {
    ...
    m_cur_rail_seg = m_cur_rail_seg->CheckCurSlideSegment(m_actor, m_actor->ar_nodes[m_sliding_node].AbsPosition);
    m_sliding_beam = m_cur_rail_seg->rs_beam->bm_pos;

    node_t& node_p1 = m_actor->ar_nodes[m_actor->ar_beams[m_sliding_beam].p1num];
    node_t& node_p2 = m_actor->ar_nodes[m_actor->ar_beams[m_sliding_beam].p2num];
...
void SlideNode::ResetPositions()
{
    ...
    {
        m_cur_rail_seg = m_cur_railgroup->FindClosestSegment(m_actor, m_actor->ar_nodes[m_sliding_node].AbsPosition);
        m_sliding_beam = (m_cur_rail_seg ? m_cur_rail_seg->rs_beam->bm_pos : BEAMID_INVALID);
        this->UpdatePosition();
    }
}

Ogre::Real SlideNode::getLenTo(const ActorPtr& actor, const RailGroup* group, const Ogre::Vector3& point)
{
    if (!group)
        return std::numeric_limits<Ogre::Real>::infinity();

    return getLenTo(actor, &group->rg_segments[0], point);
}

Ogre::Real SlideNode::getLenTo(const ActorPtr& actor, const RailSegment* rail, const Ogre::Vector3& point)
{
    if (!rail)
        return std::numeric_limits<Ogre::Real>::infinity();

    return getLenTo(actor, rail->rs_beam->bm_pos, point);
}

Ogre::Real SlideNode::getLenTo(const ActorPtr& actor, BeamID_t beamID, const Ogre::Vector3& point)
{
    if (beamID == BEAMID_INVALID)
        return std::numeric_limits<Ogre::Real>::infinity();

    node_t& node_p1 = actor->ar_nodes[actor->ar_beams[beamID].p1num];
    node_t& node_p2 = actor->ar_nodes[actor->ar_beams[beamID].p2num];

    return (NearestPointOnLine(node_p1.AbsPosition, node_p2.AbsPosition, point) - point).length();
}

Ogre::Real SlideNode::getLenTo(const RailGroup* group) const
{
    return getLenTo(m_actor, group, m_actor->ar_nodes[m_sliding_node].AbsPosition);
}

Ogre::Real SlideNode::getLenTo(const RailSegment* rail) const
{
    return getLenTo(m_actor, rail, m_actor->ar_nodes[m_sliding_node].AbsPosition);
}

Ogre::Real SlideNode::getLenTo(const beam_t* beam) const
{
    return getLenTo(m_actor, beam->bm_pos, m_actor->ar_nodes[m_sliding_node].AbsPosition);
}
...
void SlideNode::AttachToRail(RailGroup* toAttach)
{
    ...
    this->ResetPositions();
    m_cur_threshold = (m_sliding_beam != BEAMID_INVALID ? getLenTo(&m_actor->ar_beams[m_sliding_beam]) : m_initial_threshold);
}
